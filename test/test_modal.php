...
class Myform extends M5{
   ...
   htpr(br(4),
     ...
     [
      [tg('form','method="post" action="?"',
       textfield("Some text:",
               'TXTFLD',20,20,
               getpar('TXTFLD')?getpar('TXTFLD'):'Text here..',
               'id="TXTFLD"').nbsp(5).
       bt_modal_win('TXTFLD',
                  'id',
                  'klikni..',
 ...
 static function ajax(){
   ...
   /* test output value */
   foreach (['apple','apricot','ananas'] as $p ) 
  ...
  htpr(tg('script','type="text/javascript"',
   "$('.ret-value".getpar('id_window')."').click(
    function(){
     $('#".getpar('id_window')."').modal('hide');
     $('#".getpar('name')."').val(this.value); 
   // todo - schopnost nastavit korektne autocomplete polozku ...
 });"
));
