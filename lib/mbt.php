 ...
 * @return HTML string *@author Petr Čoupek
 * @package Merkur5
 * @version 1.3
 ...
 * 03.12.2024 -
 * 05.12.2024 - oprava zobrzení filtru u hlavičky listování
 * 17.03.2025 - bt_lister zrušení závislosti na parametru _whr
 *  *  * /
 *
...
function bt_container($colrules, $rows, $rowclass = 'row', $containerclass = 'container m-2')
{
    ...
    for ($i = 0; $i < count($rows); $i++) {
        ...
        for ($j = 0; $j < count($rows[$i]); $j++) {
            ...
            if (isset($rows[$i][$j])) $t .= tg('div', 'class="' . (isset($colrules[$j]) ? $colrules[$j] : '') . '"', $rows[$i][$j] . ' ');
...
function bt_hidable_area($label, $docid, $content, $addlabel = '', $hide = true)
{
    return
        tg('div', 'id="' . $docid . '_o" ',
            ta('h5', tg('a', 'id="' . $docid . '_c" href="javascript:void(0);" ' . $addlabel, $label . nbsp(2) . bt_icon('caret-down'))) .

            tg('div', 'id="' . $docid . '_i" ' . ($hide ? 'style="display:none"' : ''), $content .
                tg('a', 'id="' . $docid . '_d" href="javascript:void(0);" ' . $addlabel . ' ', bt_icon('caret-up')))) .
        ta('script',
                   ...
                   $toolbar_buttons = '')
{
    ...
    if (!$is_content) {
        ...
        $s = tg('div', 'class="table-responsive"',
            tg('table', $bt_class,
                ta('caption', $caption . nbsp(2) . 
                    ahref('?_se=1' . $context, bt_icon('search') . $text_button, 'class="btn btn-primary"') .
                    $toolbar_buttons .
                    ((isset($filter) && $filter!='')?tg('span','class="badge"',$filter):'')).
 ...
 * @param string $r - how the button looks like on the page, when default, it is a button with the $text
